-- ============================================================
-- Migration 019: Billing — Free vs Pro plans + manual approval
-- ============================================================
-- Mục đích:
--   1. Seed 2 plans: 'free' và 'pro' với price + limits
--   2. Add payment_requests table (queue admin duyệt thủ công)
--   3. Add albums.expires_at, albums.soft_deleted_at (lifecycle)
--   4. RPC helpers: get_studio_plan, can_create_album, expire_old_albums
--   5. Backfill: tạo trial Pro 30 ngày cho mọi user cũ
--
-- An toàn:
--   ✅ Idempotent — chạy lại nhiều lần OK
--   ✅ Additive — không xoá data cũ
--   ✅ Code app cũ (chưa biết Pro/Free) vẫn chạy bình thường
-- ============================================================

BEGIN;

-- ------------------------------------------------------------
-- 1. SEED plans table — Free + Pro (monthly + yearly)
-- ------------------------------------------------------------
...
CREATE INDEX IF NOT EXISTS idx_albums_soft_deleted_at ON albums(soft_deleted_at)
  WHERE soft_deleted_at IS NOT NULL;

-- ------------------------------------------------------------
-- 3. payment_requests — queue user submit, admin duyệt thủ công
-- ------------------------------------------------------------
CREATE TABLE IF NOT EXISTS payment_requests (
  ...
  reference_code TEXT NOT NULL UNIQUE,  -- ND chuyển khoản: PRO-{studio}-{ts}
...
-- ------------------------------------------------------------
-- 4. Backfill: trial Pro 30 ngày cho mọi user cũ chưa có subscription
--    + set studios.plan_id cho ai chưa có
-- ------------------------------------------------------------
DO $$
DECLARE
  v_pro_id UUID;
...
-- ------------------------------------------------------------
-- 5. RPC: get_studio_plan(studio_id) — trả plan đang active
--    Logic: nếu có subscription active/trial chưa expire → trả plan đó
--           Else trả Free.
-- ------------------------------------------------------------
...
AS $$
  WITH active_sub AS (
    SELECT sub.*, p.name AS plan_name, p.display_name, p.max_albums,
      p.max_storage_mb, p.has_zip_download, p.has_custom_branding,
  ...
  fallback_free AS (
    SELECT 'free' AS plan_name, 'Miễn phí' AS display_name, 5 AS max_albums,
      5000 AS max_storage_mb, FALSE AS has_zip_download,
      FALSE AS has_custom_branding, FALSE AS has_website_builder
  ...
  SELECT jsonb_build_object(
    ...
    'is_trial', COALESCE((SELECT status = 'trial' FROM active_sub), false),
    'expires_at', (SELECT current_period_end FROM active_sub)
  )
  FROM (SELECT * FROM active_sub UNION ALL SELECT * FROM fallback_free) x
  LIMIT 1;
$$;
...
-- ------------------------------------------------------------
-- 6. RPC: can_create_album(studio_id) — check limit trước khi insert
-- ------------------------------------------------------------
...
BEGIN
  ...
  -- Unlimited (-1) → luôn cho phép
  IF v_max = -1 THEN
    RETURN jsonb_build_object('allowed', true, 'reason', 'unlimited',
      'current', 0, 'max', -1);
  END IF;

  -- Đếm album active (chưa soft delete + chưa hard delete)
...
-- ------------------------------------------------------------
-- 7. RPC: admin_approve_payment(req_id, billing_cycle) — duyệt + activate Pro
-- ------------------------------------------------------------
...
BEGIN
  ...
  -- Tạo subscription mới (hoặc update nếu đã có active sub → extend)
  -- Đơn giản: insert mới active, đè cũ thành expired.
...
-- ------------------------------------------------------------
-- 8. RPC: expire_old_albums() — manual/cron run; soft+hard delete logic
--    Free plan: album > 30d sau create → soft delete; > 60d → hard delete
-- ------------------------------------------------------------
...
DECLARE
  ...
  v_hard_count INT := 0;
BEGIN
  -- Soft delete: album thuộc studio Free, > 30 ngày, chưa soft
  WITH freem AS (
  ...
  -- Hard delete: đã soft > 30 ngày → set deleted_at (CASCADE photos qua FK)
...
-- ------------------------------------------------------------
-- 9. RPC: list_pending_payments() — admin xem queue duyệt
-- ------------------------------------------------------------
...
-- SELECT * FROM plans;
-- SELECT s.name, sub.status, sub.current_period_end
-- FROM studios s LEFT JOIN subscriptions sub ON sub.studio_id = s.id;
-- ============================================================
